 ...
 * Bar table master.
 *
 * Table type and status are the two SQL enums the columns are declared with
 * (`bar_table_type_enum`, `bar_table_status_enum`), so they are listed here
 * rather than fetched: the database rejects anything else, and a second source
 * would only be able to disagree with it. bar_table has neither a section nor
 * a mergeable column, so neither field is offered.
 */
...
const TableMaster = () => {
  const perms = usePagePermissions("/bar_table_master");
  ...
  const handleSave = async () => {
    ...
    try {
      if (editId) {
        await APICall.putT(`/bar/table/${editId}`, {
          ...payload,
  ...
  const confirmDelete = async () => {
    ...
    setDeleteRow(null);
    try {
      await APICall.deleteT(`/bar/table/${row.id}`);
  ...
  return (
    <>
      ...
      <Modal
        ...
        onClose={closeModal}
        // In a small, single-column modal this was a form the user had to
        // scroll to reach its own Submit button.
        size="large"
        bodyLayout="grid"
        showFooter
        actions={[
      ...
      <ConfirmModal
        ...
        destructive
      >
        {`Deactivate table ${deleteRow?.table_name || ""}? It will no longer be available for orders.`}
      </ConfirmModal>
