...
const BarReceipeManagement = () => {
  const perms = usePagePermissions("/bar_recipe_management");

  const {
    data: [menuItems, inventoryItems, recipeCountRows, stations],
    loading,
    error,
    reload: load,
  } = useApiResources([
    ...
    { fetch: () => APICall.getT("/bar/inventory_item"), select: readList },
    // Returns an object keyed by menu id, not a list.
    { fetch: () => APICall.getT("/bar/menu_recipe_counts"), select: (res) => res?.data || {}, initial: {} },
  ...
  // An unknown station reads as "—" rather than "#7": a row number tells the
  // bartender nothing, and this list may simply not have loaded.
      ...
      <TableTemplate
        ...
        columns={[
          ...
          { key: "item_name", title: "Menu Item", align: "left" },
          {
            ...
            align: "left",
            type: "custom",
            render: (row) => stationName(row.station_id),
            exportValue: (row) => stationName(row.station_id),
          },
          {
            key: "id",
            title: "Ingredients",
      ...
      >
        <ViewSection title="Menu Item">
          <DetailList columns={2}>
            ...
            <DetailItem label="Station" value={stationName(selectedMenu?.station_id)} />
