 ...
 * Bar stock on hand.
 *
 * Units are the `bar_inventory_unit_enum` the column is declared with, and the two
 * adjustment reasons are the two `transaction_type` values adjust_stock
 * accepts — both are database constraints, not master data.
 */
const UNITS = ["Bottle", "Litre", "ml", "Can", "Nos"];

const ADJUST_REASONS = [
  { value: "ADJUSTMENT", label: "Correction" },
  { value: "WASTE", label: "Wastage / Breakage" },
];

const emptyAdjust = { direction: "Add", quantity: "", transaction_type: "ADJUSTMENT", remarks: "" };
const emptyItem = { item_name: "", unit: "Bottle", min_stock_level: "", is_perishable: false };

const BarStock = () => {
  const perms = usePagePermissions("/bar_stock");
    ...
    if (type === "movement") {
      ...
      try {
        const res = await APICall.getT(
          `/bar/inventory_item/${row.inventory_item_id}/transactions`,
        );
    ...
    <>
      ...
      <TableTemplate
        ...
        columns={[
          ...
          // station_name is resolved by the API. This column used to render
          // `Station #${station_id}` — a database row number shown to a
          // storekeeper who knows the place by its name.
          { key: "station_name", title: "Store", align: "left" },
          {
      ...
      >
        <ViewSection title="Item">
          <DetailList columns={2}>
            ...
            <DetailItem label="Store" value={selectedItem?.station_name} />
          </DetailList>
        </ViewSection>
      ...
      >
        ...
        <Input
          ...
          required
          name="item_name"
          placeholder="e.g. Old Monk 750ml"
        ...
        />
        {/* Was a two-option "Yes"/"No" <Select> for a boolean column — a
            hand-rolled stand-in for the Switch the design system already
            ships. */}
        <Switch
          label="Perishable"
          checked={newItemForm.is_perishable}
          onChange={(e) => setNewItemForm((p) => ({ ...p, is_perishable: e.target.checked }))}
        />
      </Modal>
