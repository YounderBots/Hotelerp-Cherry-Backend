...
const UNIT_OPTIONS = ["Litre", "ml", "Kg", "Gram", "Nos"];
const YES_NO_OPTIONS = [{ value: "false", label: "No" }, { value: "true", label: "Yes" }];
...
const BarStock = () => {
  const {
    ...
    setError,
    reload: load,
  } = useApiResource(() => APICall.getT("/bar/inventory_stock"), {
    select: readList,
  ...
  const [newItemForm, setNewItemForm] = useState({ item_name: "", unit: "Litre", min_stock_level: "", is_perishable: false });
  ...
  const openModal = async (type, row) => {
    ...
    if (type === "movement") {
      try {
        const res = await APICall.getT(`/bar/inventory_item/${row.inventory_item_id}/transactions`);
  ...
  const openAddItemModal = () => {
    setNewItemForm({ item_name: "", unit: "Litre", min_stock_level: "", is_perishable: false });
    setFormError(null);
  ...
  const saveNewItem = async () => {
    ...
    setFormError(null);
    try {
      await APICall.postT("/bar/inventory_item", {
  ...
  const saveAdjustment = async () => {
    ...
    try {
      ...
      await APICall.postT("/bar/inventory_stock/adjust", {
        inventory_item_id: selectedItem.inventory_item_id,
        station_id: selectedItem.station_id || null,
        quantity: signed,
  ...
  return (
    <>
      <div style={{ marginBottom: 16 }}>
        <h3 style={{ marginBottom: 4 }}>Bar Stock Management</h3>
        <div style={{ fontSize: 13, color: "#64748b" }}>Bar → Inventory → Stock</div>
      </div>
      ...
      <TableTemplate
        ...
        columns={[
          ...
          { key: "station_id", title: "Store", align: "center", type: "custom", render: (row) => (row.station_id ? `Station #${row.station_id}` : "Main Store") },
      ...
      {activeModal === "view" && selectedItem && (
        ...
        >
          <p><strong>Store:</strong> {selectedItem.station_id ? `Station #${selectedItem.station_id}` : "Main Store"}</p>
      ...
      {activeModal === "add-item" && (
        ...
        >
          ...
          <Select
            label="Perishable"
            value={String(newItemForm.is_perishable)}
            onChange={(e) => setNewItemForm((p) => ({ ...p, is_perishable: e.target.value === "true" }))}
      ...
      {activeModal === "adjust" && selectedItem && (
        ...
        >
          ...
          <Select
            ...
            options={[{ value: "ADJUSTMENT", label: "Correction" }, { value: "WASTE", label: "Wastage / Breakage" }]}
          />
          <div className="form-group">
            <label className="form-label">Remarks</label>
            <textarea className="form-control" rows="2" value={adjustForm.remarks} onChange={(e) => setAdjustForm((p) => ({ ...p, remarks: e.target.value }))} />
          </div>
        </Modal>
      )}
