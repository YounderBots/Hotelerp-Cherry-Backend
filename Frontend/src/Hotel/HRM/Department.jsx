...
const Department = () => {
  const { data, loading, error, reload } = useApiResource(
    ...
    { select: readList, fallback: "Failed to load departments." },
  );

  const permissions = usePagePermissions("/department");
  ...
  const handleEdit = (row) => {
    setFormData({ name: row.department_name ?? "" });
    setEditId(row.id);
    setShowModal(true);
  };
  ...
  const handleSave = async () => {
    ...
    if (name.length > 100) {
      showToast("Department Name must be under 100 characters", "error");
      return;
    }
  ...
  return (
    <>
      ...
      <TableTemplate
        title="Departments"
        loading={loading}
        emptyMessage="No departments yet. Add the first one to get started."
        ...
        columns={[
          { key: "department_name", title: "Department Name", align: "left" },
          {
            key: "actions",
            title: "Actions",
            align: "center",
      ...
      >
        <DetailList columns={1}>
          <DetailItem label="Department Name" value={viewData?.department_name} />
        </DetailList>
      </Modal>
      ...
      <ConfirmModal
        ...
        destructive
      >
        Are you sure you want to delete this department? Employees
