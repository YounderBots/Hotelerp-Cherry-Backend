 ...
 *
 * These are page defaults, not master data. RestaurantServices does expose a
 * generic settings key/value store (GET /restaurant/settings), but it has no
...
const BillingPayments = () => {
  const perms = usePagePermissions("/billing_payments");
  ...
  const {
    ...
    {
      fetch: () => APICall.getT("/restaurant/bill"),
      select: readList,
      fallback: "Failed to load bills.",
    },
    { fetch: () => APICall.getT("/restaurant/order"), select: readList },
    { fetch: () => APICall.getT("/restaurant/payment_method"), select: readList },
  ...
  const {
    ...
    error: genItemsError,
  } = useApiResource(() => APICall.getT(`/restaurant/order/${genOrderId}`), {
  ...
  const submitCancelBill = async () => {
    ...
    setSaving(true);
    try {
      await APICall.putT(`/restaurant/bill/${selectedBill.id}/cancel`, {
  ...
  const printBill = () => {
    ...
    const ok = printDocument({
      ...
      body:
        ...
        printRow("Table / Room", b.table_code || b.room_no || "—") +
        printHeading("Items") +
      ...
      >
        <ViewSection title="Bill">
          <DetailList columns={3}>
            ...
            <DetailItem
              label="Table / Room"
              value={selectedBill?.table_code || selectedBill?.room_no}
            />
