...
class Room(Base):
    ...
    Room_Booking_status= Column(String(100), nullable=False, index=True) #Occupied, Available
    Room_Working_status = Column(String(100), nullable=False, index=True) #(Default : Room_Booking_status , Task Type db data ex: Cleaning, etc.., if task over details store Not Assigne)
    Room_Status = Column(String(100), nullable=False, index=True)  #(Blocking,UnBlocking)
    status = Column(String(100), nullable=False, index=True)
    created_by = Column(String(100), nullable=False, index=True)
    created_at = Column(DateTime, server_default=func.now())
    updated_at = Column(DateTime, onupdate=func.now())
