...
class ShiftIn(BaseModel):
    ...
    role: str  # Bartender | Cashier | Manager
    shift_date: date
    shift_start: time
...
def list_shifts(
    ...
    user_id, role_id, company_id = _auth(request)
    q = db.query(models.BarStaffAssignment).filter(models.BarStaffAssignment.company_id == company_id, models.BarStaffAssignment.status == STATUS)
    if shift_date is not None:
        q = q.filter(models.BarStaffAssignment.shift_date == shift_date)
    if employee_id is not None:
        q = q.filter(models.BarStaffAssignment.employee_id == employee_id)
    rows = q.order_by(models.BarStaffAssignment.shift_date.desc(), models.BarStaffAssignment.shift_start.asc()).all()
