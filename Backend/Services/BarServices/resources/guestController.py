...
def create_guest(payload: GuestIn, request: Request, db: Session = Depends(get_db)):
    ...
    existing = db.query(models.BarGuest).filter(models.BarGuest.mobile == payload.mobile, models.BarGuest.company_id == company_id, models.BarGuest.status == STATUS).first()
    if existing:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="A guest with this mobile number already exists")
    guest = models.BarGuest(guest_code=gen_code("BGST"), created_by=user_id, company_id=company_id, **payload.dict())
    db.add(guest)
    db.commit()
    db.refresh(guest)
...
def get_guest(guest_id: int, request: Request, db: Session = Depends(get_db)):
    user_id, role_id, company_id = _auth(request)
    guest = db.query(models.BarGuest).filter(models.BarGuest.id == guest_id, models.BarGuest.company_id == company_id).first()
    if not guest:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Guest not found")
    addresses = db.query(models.BarGuestAddress).filter(models.BarGuestAddress.guest_id == guest_id, models.BarGuestAddress.status == STATUS).all()
    history = db.query(models.BarGuestVisitHistory).filter(models.BarGuestVisitHistory.guest_id == guest_id).order_by(models.BarGuestVisitHistory.visit_date.desc()).all()
...
def add_guest_address(guest_id: int, payload: AddressIn, request: Request, db: Session = Depends(get_db)):
    ...
    if not guest:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Guest not found")
    addr = models.BarGuestAddress(guest_id=guest_id, company_id=company_id, **payload.dict())
    db.add(addr)
    db.commit()
    db.refresh(addr)
