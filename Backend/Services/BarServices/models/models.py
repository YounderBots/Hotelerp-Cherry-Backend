...
class BarBillPayment(Base):
    ...
    # ix_bar_bill_status. MySQL scopes index names per table so production
    ...
    __table_args__ = (Index("ix_bar_bill_payment_lifecycle_status", "status"),)
    ...
    status = Column(SAEnum(*STATUS_VALUES, name="bar_bill_payment_status_lifecycle_enum"), nullable=False, default="ACTIVE")
    created_by = Column(String(100), nullable=False)
    created_at = Column(DateTime, server_default=func.now())
    updated_at = Column(DateTime, onupdate=func.now())
    updated_by = Column(String(100), nullable=True)
