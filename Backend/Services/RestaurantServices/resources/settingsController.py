...
def create_setting(payload: SettingIn, request: Request, db: Session = Depends(get_db)):
    ...
    existing = (
        db.query(models.RestaurantSettings)
        .filter(models.RestaurantSettings.setting_key == payload.setting_key, models.RestaurantSettings.company_id == company_id)
        .first()
    )
    if existing:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="setting_key already exists; use PUT to update it")
    row = models.RestaurantSettings(created_by=user_id, company_id=company_id, **payload.dict())
    db.add(row)
    db.commit()
    db.refresh(row)
...
def list_settings(request: Request, setting_group: Optional[str] = Query(None), db: Session = Depends(get_db)):
    user_id, role_id, company_id = _auth(request)
    q = db.query(models.RestaurantSettings).filter(models.RestaurantSettings.company_id == company_id, models.RestaurantSettings.status == STATUS)
    if setting_group:
        q = q.filter(models.RestaurantSettings.setting_group == setting_group)
    rows = q.order_by(models.RestaurantSettings.setting_group.asc(), models.RestaurantSettings.setting_key.asc()).all()
...
def update_setting(setting_key: str, payload: SettingUpdate, request: Request, db: Session = Depends(get_db)):
    ...
    row = (
        db.query(models.RestaurantSettings)
        .filter(models.RestaurantSettings.setting_key == setting_key, models.RestaurantSettings.company_id == company_id)
        .first()
    )
    if not row:
