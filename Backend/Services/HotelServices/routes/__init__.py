...
from resources.frontOffice.RoomIncidentLogController import router as roomincidentlog

router = APIRouter()
 
router.include_router(housekeepingRouter, prefix='', tags=['Housekeeping']) 
router.include_router(guestRouter, prefix='', tags=['Guest'])
router.include_router(roomincidentlog, prefix='', tags=['Roomincidentlog'])