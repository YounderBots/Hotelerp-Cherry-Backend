    ...
    if "sessid" in request.session:
        ...
        try:
            ...
            if created_by is None:
                ...
                return templates.TemplateResponse('restaurant/inventory/stock.html', context={'request': request,'color_theme':color_theme,'inquiry_data':inquiry_data,'permision_control':permision_control,
    ...
    if "sessid" in request.session:
        ...
        try:
            ...
            if created_by is None:
                ...
                loginer_data = db.query(models.Employee_Data).filter(models.Employee_Data.id==created_by).filter(models.Employee_Data.status==CommonWords.STATUS).first()
                inquiry_data = db.query(models.Inquiry).filter(models.Inquiry.status==CommonWords.STATUS).all()
                return templates.TemplateResponse('restaurant/inventory/recipe_management.html', context={'request': request,'color_theme':color_theme,'inquiry_data':inquiry_data,'permision_control':permision_control,
                                                                                          'loginer_data':loginer_data,})
        except JWTError:
            return RedirectResponse(CommonWords.LOGINER_URL, status_code=status.HTTP_307_TEMPORARY_REDIRECT)
    else:
