...
|---|---|---|
| 1 | `/room_reservation` and `/room_reservation/{id}` answer 500 | Deploy the 18 Sept build: the Hotel service now reads Master Data and Users over HTTP and never touches their schemas, so no GRANT is needed. Run `python Backend/migrations/migrate.py upgrade hotel` (adds `room_lock`), set `MASTER_SERVICE_URL` / `USER_SERVICE_URL` in the Hotel `.env`, restart; `/readyz` names any sibling it cannot reach |
